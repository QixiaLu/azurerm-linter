...
func invalidCases() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"config_single": { // want `AZC004: field "config_single" has MaxItems: 1 with only one nested property - consider flattening or add inline comment explaining why \(e.g., '// Additional properties will be added per service team confirmation'\)`
			Type:     schema.TypeList,
			Optional: true,
			MaxItems: 1,
			Elem: &schema.Resource{
		...
		"settings": { // want `AZC004: field "settings" has MaxItems: 1 with only one nested property - consider flattening or add inline comment explaining why \(e.g., '// Additional properties will be added per service team confirmation'\)`
			Type:     schema.TypeList,
			Optional: true,
			MaxItems: 1,
			Elem: &schema.Resource{
