...
func invalidCases() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"resource_group_name": { // want `AZC002: string argument "resource_group_name" must have ValidateFunc`
			Type:     schema.TypeString,
			Required: true,
		},

		// Invalid: Optional String without ValidateFunc
		"location": { // want `AZC002: string argument "location" must have ValidateFunc`
			Type:     schema.TypeString,
			Optional: true,
		},

		// Invalid: Required String with other fields but no ValidateFunc
		"sku": { // want `AZC002: string argument "sku" must have ValidateFunc`
			Type:     schema.TypeString,
			Required: true,
			ForceNew: true,
		},
