...
// LoadChanges sets up the changed lines tracking system
func LoadChanges() error {
	mu.Lock()
	defer mu.Unlock()
...
func selectGitLoader() ChangeLoader {
	if isGitHubActions() && canUseGitHubAPI() {
		log.Println("Detected GitHub Actions with PR context")
		return &GitHubLoader{}
	}
	...
	if *useGitHubAPI {
		...
		log.Println("Warning: GitHub API mode requires -pr-number, falling back to local git")
	}
